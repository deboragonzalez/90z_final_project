...
stargazer(m1, m2, m1.5, m2.5, digits = 3, 
          ...
          dep.var.labels = c("Family Income in 2016", "Highest Grade by Age 25"),
          header = F,
          table.placement = "h",
...
```{r direct_effect, warning = FALSE}

# plot m2.5 using child's year of schooling on the y axis
...
augment(m2.5) %>%
  ...
  geom_ribbon(aes(ymin = .fitted - 1.96 * .se.fit,
                  ymax = .fitted + 1.96 * .se.fit),
              alpha = 0.3) +
  labs(title = "Direct Effect of Mother's Education",
       subtitle = "Mediated by Sex, Family Income, and Father's Education",
       x = "Highest Grade Completed by Mother in 1979",
       y = "Predicted Child's Highest Grade by Age 25") +
  theme_classic()
...
Figure 2 visualizes the relationship between the two outcomes of interest and father's education. It can be compared to Figure 1, which shows the mother's effect.
